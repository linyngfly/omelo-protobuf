#Pomelo-protobuf
  Protobuf protocol is a high efficient binary protocol for data encode, this module implement the protobuf protocol, and used in [omelo](https://github.com/NetEase/omelo) for data transfer.
Of course, omelo-protobuf can also be used independently in other projects.
##Architecture
Unlike the google protobuf, we provide a universal encoder and decoder in omelo-protobuf. We use protos file as meta data to encode/decode messages, so you do not need to add any code to your project, instead , what you need is to add a protos.json (or two for different encoder and decoder messages) files to define the message need to encode by protobuf.The architecture of omelo-protobuf is as follow:

![omelo protobuf](http://omelo.netease.com/resource/documentImage/protocol/Protobuf_pomelo.png)

##Usage
###Define protos
To use omelo-protobuf, you need to write a JSON file to define the message format. The syntax of the file is as the same as the .proto file in protobuf, but in JSON format, here is the example protos.json:
...
###Compatibility
For the same message and proto, the encode results are **the same** for **omelo-protobuf** and **google protobuf** .This means you can exchange binary data with google-protobuf.
